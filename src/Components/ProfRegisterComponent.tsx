...
export default class ProfRegisterComponent extends Component<ScreenInterfcae, RegisterUserInterface>{
    ...
    async getApiData(){
        await CommonApiRequest.getProfCategory().then((response)=>{
            this.setState({loader:false});
            if(response?.status===200){
                this.setState({catData:response?.results});
            }
        }).catch(()=>{
            this.setState({loader:false})
        })
    ...
    checkValidation() {
        if (this.state.name !== '' && this.state.email !== '' && this.state.password !== '' && this.state.c_password !== ''  && this.state.selectedCat !== ''  && this.state.location !== '') {
                    ...
                    <View style={{ height: '100%' }}>
                        <ScrollView contentContainerStyle={[ThemeStyling.container, { flex: 1 }]}>
                            <View style={{ marginBottom: 'auto', marginTop: 'auto' }}>
                                ...
                                <View>
                                    <FormGroup>
                                        <SelectList boxStyles={{borderRadius: 10,borderStyle: 'solid',borderColor: Colors.gray400,minHeight:55,alignItems:'center'}} setSelected={(val)=>{this.setState({selectedCat:val})}} data={this.state.catData} save="key" search={false}/>
                                    </FormGroup>
                                </View>
                                <View>
                                    <FormGroup>
                                        <InputComponent attrName={'location'} textContentType={'none'} secureTextEntry={false} value={this.state?.location} placeholder={"Address"} updateMasterState={(attr: any, value: any) => { this.upDateMasterState(attr, value) }}></InputComponent>
                                    </FormGroup>
                                </View>
